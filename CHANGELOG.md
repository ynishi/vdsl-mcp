...
- **`vdsl_cam_lua_init`** — Scaffold a new cam Lua script at `<root>/<persona_id>_cam_<topic>.lua`. Resolves a persona-specific base subject snippet via a 3-tier fallback (`~/.claude/skills/cam-vdsl/bases/<persona_id>.lua` → `~/.claude/skills/snap-<persona_id>/<persona_id>_base.lua` → thin default) so the generated skeleton is always persona-aware. Accepts optional `shots` (`CamShotSpec[]` with `name`, `seed`, `trait_dsl`) and `topic`; omitting either triggers sensible defaults. Creates the target directory automatically (`std::fs::create_dir_all`) and returns the absolute `script_file` path — callers never need a prior `mkdir` step or a `Write` tool. `overwrite=false` default refuses to replace an existing file. Returns `base_source` indicating which resolution path was used (`file:<path>` or `fallback:default`).
